...
> 当前 GitHub 仓库已公开，任何人都可以下载，并按[安装与使用](#安装与使用)完成安装。项目目前仍为 `UNLICENSED`：公开可见和可下载不等于取得复制、修改或再分发许可；如需开放复用，还需要另行添加许可证。
...
8. **尚未确定开源许可证**：当前清单为 `UNLICENSED`。仓库已经公开可见和下载，但尚未授予公众复制、修改和再分发许可；如需开放复用，需要作者选择并加入许可证。
